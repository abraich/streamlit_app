...
class Sensitivity():
    ...
    def sensitivity_analysis(self):
        ...
        psi_t = model.get_repr(x_train)

        # get the representation of the data
        psi_t_test = model.get_repr(x_test)

        # get the prediction of the model
        surv_pred = model.predict(x_test)
        
        # compute the sensitivity
        sens_s = get_sensitivity_score(
            psi_t, psi_t_test, durations_test, events_test)
        sens_s = sens_s.sort_values(ascending=False)
        return sens_s
