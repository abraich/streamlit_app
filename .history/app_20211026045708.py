...
if task == 'WD vs n_samples':
    if task == 'Simulation':
        ...
        n_samples_min = st.sidebar.number_input(
            "n_samples", min_value=1000)
        n_samples_max = st.sidebar.number_input(
            "n_samples", min_value=2000)
        steps = st.sidebar.number_input("steps", min_value=10)
        
        # Simulation of data
        for i in np.linspace(n_samples_min,n_samples_max,steps):
            params["n_samples"]=i
        simu = Simulation(params)
