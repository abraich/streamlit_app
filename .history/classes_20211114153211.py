...
class Tunning():
    ...
    def objective_bart(self, trial):
        ...
        model0, model1 =BART(num_trees = self.params_search['num_trees']),BART(num_trees = self.params_search['num_trees'])
        model0.fit(X= self.Eval.data.x_0_train,
                    T= self.Eval.data.T_f_0_train,E= self.Eval.data.e_0_train,max_features = self.params_search['max_features'], 
                    max_depth = self.params_search['max_depth'], alpha = self.params_search['alpha'])
        model1.fit(X= self.Eval.data.x_1_train,
                    T= self.Eval.data.T_f_1_train,E= self.Eval.data.e_1_train,max_features = self.params_search['max_features'], 
                    max_depth = self.params_search['max_depth'], alpha = self.params_search['alpha'])
        #d_train = Eval.Results_SurvCaus(SC, is_train=True)
        d_val = self.Eval.Results_BART(model0, model1, is_train=False)
        #MMise_train = (np.mean(d_train["mise_0"])+np.mean(d_train["mise_1"]))/2
        MMise_val = (np.mean(d_val["mise_0"])+np.mean(d_val["mise_1"]))/2
    ...
    def get_best_hyperparameter_bart(self, n_trials=10):
        self.study = optuna.create_study(direction='minimize')
        self.study.optimize(self.objective_bart, n_trials)
        self.best_trial_values = self.study.best_trial.values
        self.best_trial_params = self.study.best_trial.params
        return self.best_trial_params
