...
class Tunning():
    ...
    def objective_bart(self, trial):
        ...
        bart = BART(self.params_search['num_trees'])
        model0, model1 =BART(self.params_search['num_trees']),BART(self.params_search['num_trees'])
        model0.fit(self.Eval.data.x_0_train,
                    self.Eval.data.T_f_0_train, self.Eval.data.e_0_train)
        model1.fit(self.Eval.data.x_1_train,
                    self.Eval.data.T_f_1_train, self.Eval.data.e_1_train)
        
        d_val = self.Eval.Results_Benchmark(model0, model1)
        MMise_val = (np.mean(d_val["mise_0"])+np.mean(d_val["mise_1"]))/2
        return MMise_val
