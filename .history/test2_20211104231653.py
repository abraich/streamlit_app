...
class EMA_strategy:
    def __init__(self, coin, interval, date1, date2,binance):
        ...
        self.strategy = 'EMA'
    def get_data(self):
        self.df = self.binance.get_data_by_coin_df(self.coin, self.interval, self.date1, self.date2)
    ...
    def plot_ema_prediction(self):
        ...
        # plot
        py.plot(data, filename='ema_prediction_plotly')        df = self.df
        df['close'] = self.df['close'].astype(float)
        df['ema'] = df['close'].ewm(span=5, min_periods=5, adjust=False).mean()
        df['ema'] = df['ema'].tolist()
        #df['ema'] = df['ema'][-1]
        ...
        data = [trace1, trace2]
        layout = dict(title='EMA Prediction',
...
"""
# test EMA strategy
E = EMA_strategy('SHIBUSDT', Client.KLINE_INTERVAL_1MINUTE, '1 day ago UTC', 'now UTC',B)
print(E.get_data())
E.get_ema_list(5)
