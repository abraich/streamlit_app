...
class Binance : 
    ...
    def get_data_by_coin(self, coin, interval):
        list_d = self.client.get_historical_klines(symbol=coin, interval=interval, start_str='1 day ago UTC')
        df = pd.DataFrame(list_d, columns=['Open time', 'Open', 'High', 'Low', 'Close', 'Volume', 'Close time', 'Number of trades', 'Taker buy base asset volume', 'Taker buy quote asset volume', 'Ignore'])
        df = df.drop(['Taker buy base asset volume','Taker buy base asset volume','Ignore'], axis=1)
        return  df 
