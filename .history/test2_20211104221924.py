...
class Binance : 
    ...
    def get_data_by_coin_df(self, coin, interval,date1,date2):
        ...
        return pd.DataFrame(self.client.get_historical_klines(symbol=coin, interval=interval, start_str=date1, end_str=date2), columns=colum_names)

    def get_data_by_coin_df_all(self, coin, interval):
        colum_names = ['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_asset_volume', 'number_of_trades', 'taker_buy_base_asset_volume', 'taker_buy_quote_asset_volume', 'ignore']
        return pd.DataFrame(self.client.get_historical_klines(symbol=coin, interval=interval, start_str='1 day ago UTC', end_str='now UTC',columns=colum_names))
...
print(B.get_data_by_coin_df('BTCUSDT', Client.KLINE_INTERVAL_1MINUTE, '1 day ago UTC', 'now UTC'))
