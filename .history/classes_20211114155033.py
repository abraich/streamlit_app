...
class Sensitivity():
    ...
    def sensitivity_analysis(self):
        ...
        # define the model
        model = SurvivalModel(
            self.params_sim, self.params_survcaus).get_model()
...
class Evaluation():
    def __init__(self, params_simu, params_survcaus, params_bart):
    ...
    def All_Results(self, list_models=["SurvCaus", "SurvCaus_0"], is_train=True):
        ...
        for model_name in list_models:
            ...
            if model_name == 'BART':
                model0, model1 = BART(num_trees=self.params_search['num_trees']), BART(
                    num_trees=self.params_search['num_trees'])
                model0.fit(X=self.data.x_0_train,
                           T=self.data.T_f_0_train, E=self.data.e_0_train, max_features=self.params_search[
                               'max_features'],
                           max_depth=self.params_search['max_depth'], alpha=self.params_search['alpha'])
                model1.fit(X=self.data.x_1_train,
                           T=self.data.T_f_1_train, E=self.data.e_1_train, max_features=self.params_search[
                               'max_features'],
                           max_depth=self.params_search['max_depth'], alpha=self.params_search['alpha'])
...
class Tunning():
    ...
    def objective_bart(self, trial):
        ...
        model0.fit(X=self.Eval.data.x_0_train,
                   T=self.Eval.data.T_f_0_train, E=self.Eval.data.e_0_train, max_features=self.params_search[
                       'max_features'],
                   max_depth=self.params_search['max_depth'], alpha=self.params_search['alpha'])
        model1.fit(X=self.Eval.data.x_1_train,
                   T=self.Eval.data.T_f_1_train, E=self.Eval.data.e_1_train, max_features=self.params_search[
                       'max_features'],
                   max_depth=self.params_search['max_depth'], alpha=self.params_search['alpha'])
        #d_train = Eval.Results_SurvCaus(SC, is_train=True)
