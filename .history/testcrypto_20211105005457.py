...
def scrape_coins():
    ...
    for coin in binance_data:
        if 'USDT' in coin['symbol']:
            ...
            print(coin['symbol'], price_change)
        
    print("Gate.io coins: ", gate_coins)
    print("Binance coins: ", binance_coins)
    diff = list(set(gate_coins) - set(binance_coins))
    print("Difference: ", diff,len(gate_coins),len(binance_coins),len(diff))
...
# define a function scrape 10 best performing change price coins on gate.io
