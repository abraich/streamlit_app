...
if task == 'Simulation':
    ...
    # random array [1,2, 3] , len = n_features
    idx =np.random.choice(np.arange(1, 4), size=n_features)  #np.arange(n_features)
   
    params['beta'] = (-1) ** idx  * np.exp(-idx*2 )
