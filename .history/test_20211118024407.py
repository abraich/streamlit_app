...
def table(models_list, d_exp):
    n = len(models_list)
    mise_0 = [d_exp[f'mise_0_{model_name}'].mean() for model_name in models_list]
    mise_1 = [d_exp[f'mise_1_{model_name}'].mean() for model_name in models_list]
    CATE = [d_exp[f'CATE_{model_name}'].mean() for model_name in models_list]
    df = pd.DataFrame({'Models': models_list,
                       'Mise_0': mise_0,
                       'Mise_1': mise_1,
                       'CATE': CATE})
