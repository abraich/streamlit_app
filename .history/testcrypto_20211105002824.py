...
class CryptoTrader:
    ...
    # defin a strategy
    def strategy_1(self, logger, client, ws, symbol, strategy_parameters):
    def set_strategy(self, strategy, strategy_parameters):
        self.strategy = strategy(self.logger, self.client, self.ws, self.symbol, strategy_parameters)
    def bot_loop(self):
        while True:
            try:
                self.logger.info("Running Strategy")
                self.strategy.run()
                self.logger.info("Running Strategy - Done")
