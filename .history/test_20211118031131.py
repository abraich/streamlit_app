...
def table(models_list, d_expn,WD):
    ...
    df_table.columns.name = f'WD = {int(WD)}'
    # display the table
    return df_table
    
# concatenate tables
def concatenate(df_table_list):
    # empty spaces between tables    
    df_table_list[0] = df_table_list[0].rename(columns={f'WD = {int(WD)}': f'WD = {int(WD)}'})
    df_table_list[0].columns.name = f'WD = {int(WD)}'
    for i in range(len(df_table_list) - 1):
        df_table_list[i + 1] = df_table_list[i + 1].rename(columns={f'WD = {int(WD)}': f'WD = {int(WD)}'})
        df_table_list[i + 1].columns.name = f'WD = {int(WD)}'
        df_table_list[i + 1] = df_table_list[i + 1].add_prefix(f'WD = {int(WD)} ')
