...
class Grid_Trading :
    def __init__(self, binance, coin, interval, price_base, price_increment, quantity):
        ...
        self.df = binance.get_data_by_coin_df(coin, interval, '1 day ago UTC', 'now UTC')
        self.df['price_buy'] = self.df['close'] + self.price_increment
        self.df['price_sell'] = self.df['close'] - self.price_increment
    def get_df(self):
        return self.df
    ...
    def set_price_increment(self, price_increment):
        self.price_increment = price_increment
    def set_quantity(self, quantity):
        self.quantity = quantity
    def get_price_buy(self):
        return self.df['price_buy']
    def get_price_sell(self):
        return self.df['price_sell']
    def buy(self):
        self.binance.buy_coin(self.coin, self.quantity, self.price_base)
    def sell(self):
        self.binance.sell_coin(self.coin, self.quantity, self.price_base)
    def grid_buy(self):
        for i in range(len(self.df)):
            self.binance.buy_coin(self.coin, self.quantity, self.df['price_buy'][i])
    def grid_sell(self):
        for i in range(len(self.df)):
            self.binance.sell_coin(self.coin, self.quantity, self.df['price_sell'][i])
