...
class Sensitivity():
    ...
    def sensitivity_analysis(self):
        ...
        for i in range(x_train_.shape[1]):
            ...
            S_pred_train = model.predict(x_train_)
            S_pred_val = model.predict(x_val_)
            S_pred_test = model.predict(x_test_)
            # compute the sensitivity
            sens_train.append(
            
    
def get_sensitivity_score(psi_t, psi_t_test, durations_test, events_test):
    # compute the sensitivity
    sens_s = np.zeros(psi_t.shape[1])
    for i in range(psi_t.shape[1]):
        psi_t_i = psi_t[:, i]
        psi_t_test_i = psi_t_test[:, i]
        sens_s[i] = get_sensitivity_score_i(
            psi_t_i,
            psi_t_test_i,
            durations_test,
            events_test,
    ...
    return pd.Series(sens_s, index=psi_t.columns)
def get_sensitivity_score_i(psi_t_i, psi_t_test_i, durations_test, events_test):
    # compute the sensitivity
    sens_s_i = 0
    for i in range(len(psi_t_i)):
        sens_s_i += np.abs(psi_t_i[i] - psi_t_test_i[i])

    sens_s_i = sens_s_i / len(psi_t_i)
...
class Tunning():
    ...
    def get_best_hyperparameter(self, n_trials=10):
