...
if task == 'Simulation':
    ...
    if scheme == 'linear':
        ...
        st.sidebar.markdown(r"$f_1$ : $sin\left(\pi\times X_1\times X_2\right) + 2\left(X_3 - 0.5\right)^2 + X_4 + 0.5X_5$")
        st.sidebar.markdown(r"$f_2$ : $2\log\left(1 + e^{X_1 + X_2 + X_3}\right)$")
        st.sidebar.markdown(r"$f_3$ : $\frac{\max\left(X_1 + X_2 + X_3, 0\right) + \max\left(X_4 + X_5, 0\right)}{2}$")
        st.sidebar.markdown(r"$f_4$ : $\max\left(X_1 + X_2, X_3, 0\right) + \max\left(X_4 + X_5, 0\right)$")
    ...
    st.write("WD : ", simu.wd)
    st.write('Cov matrix : ', simu.cov_matrix)
    # TSNE
