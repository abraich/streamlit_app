...
def table(models_list, d_exp):
    n = len(models_list)
    mise_0 = [d_exp[f'mise_0_{model_name}'] for model_name in models_list]
    mise_1 = [d_exp[f'mise_1_{model_name}'] for model_name in models_list]
    CATE = [d_exp[f'CATE_{model_name}'] for model_name in models_list]
    table = np.vstack((mise_0, mise_1, CATE))
    ...
    df = pd.DataFrame(np.transpose(table), index=models_list, columns=['mise_0', 'mise_1', 'CATE'])
    print(df)
