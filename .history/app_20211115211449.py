...
session_state1 = SessionState.get(name='', params_best_survcaus=None)  # cache the session state

session_state2 = SessionState.get(name='', params_best_bart = None)  # cache the session state
...
if task == 'Simulation':
    ...
    if scheme == 'linear':
        ...
        st.sidebar.markdown("f1 : sin(πXi1Xi2) + 2(Xi3 − 0.5)2 + Xi4 + 0.5Xi5")
        st.sidebar.markdown("f2 : 2 log(1 + exp(Xi1+Xi2+Xi3 )")
        st.sidebar.markdown("f3 : (max{Xi1 + Xi2 + Xi3, 0} + max{Xi4 + Xi5, 0}) /2")
        st.sidebar.markdown("f4 : max{Xi1 + Xi2, Xi3, 0} + max{Xi4 + Xi5, 0}")
        f_choices = ['f1', 'f2', 'f3', 'f4']
        f_choice = st.sidebar.selectbox("Select function", f_choices)
        if f_choice == 'f1':
            params['scheme'] = Scheme('nonlinear', f=f1)
        elif f_choice == 'f2':
            params['scheme'] = Scheme('nonlinear', f=f2)
        elif f_choice == 'f3': 
            params['scheme'] = Scheme('nonlinear', f=f3)
        elif f_choice == 'f4':
            params['scheme'] = Scheme('nonlinear', f=f4)
